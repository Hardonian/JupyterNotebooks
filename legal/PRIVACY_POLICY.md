# Privacy Policy

**Last Updated:** [Date]  
**Effective Date:** [Date]  
**Version:** 1.0

---

## 1. Introduction

Agent Factory ("we," "our," or "us") is committed to protecting your privacy. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our Service.

---

## 2. Information We Collect

### 2.1 Information You Provide

- **Account Information:** Name, email, password
- **Profile Information:** Company, role, preferences
- **Content:** Agents, workflows, blueprints you create
- **Payment Information:** Processed by Stripe (we don't store full card details)

### 2.2 Information Automatically Collected

- **Usage Data:** API calls, features used, timestamps
- **Device Information:** IP address, browser type, device type
- **Logs:** Request logs, error logs, performance metrics
- **Cookies:** Session cookies, analytics cookies

### 2.3 Information from Third Parties

- **Authentication Providers:** If you use OAuth/SSO
- **Payment Processors:** Stripe (transaction data)
- **Analytics:** Usage analytics (if enabled)

---

## 3. How We Use Your Information

### 3.1 Service Provision

- Provide and maintain the Service
- Process transactions
- Send service-related communications
- Provide customer support

### 3.2 Improvement

- Improve the Service
- Develop new features
- Analyze usage patterns
- Conduct research

### 3.3 Communication

- Send updates and announcements
- Respond to inquiries
- Send marketing communications (with consent)
- Send security alerts

### 3.4 Legal Compliance

- Comply with legal obligations
- Enforce our Terms of Service
- Protect our rights
- Prevent fraud

---

## 4. How We Share Your Information

### 4.1 Service Providers

We share information with:

- **Hosting:** Vercel, Supabase
- **Payment Processing:** Stripe
- **Analytics:** [Analytics Provider]
- **Email:** [Email Provider]

### 4.2 Legal Requirements

We may disclose information if required by law or to:

- Comply with legal obligations
- Protect our rights
- Prevent harm
- Respond to government requests

### 4.3 Business Transfers

In the event of a merger, acquisition, or sale, your information may be transferred.

### 4.4 With Your Consent

We share information with your explicit consent.

---

## 5. Data Security

### 5.1 Security Measures

- Encryption in transit (TLS)
- Encryption at rest
- Access controls
- Regular security audits
- Secure authentication

### 5.2 Data Breaches

- We will notify you of data breaches
- Notification within 72 hours (GDPR requirement)
- We will take steps to mitigate impact

---

## 6. Your Rights (GDPR)

### 6.1 Right to Access

You can request a copy of your personal data.

**How to Exercise:** Contact privacy@agentfactory.io or use API endpoint

### 6.2 Right to Rectification

You can update your personal data.

**How to Exercise:** Update in account settings or contact us

### 6.3 Right to Erasure

You can request deletion of your personal data.

**How to Exercise:** Delete account or contact privacy@agentfactory.io

### 6.4 Right to Restrict Processing

You can request restriction of processing.

**How to Exercise:** Contact privacy@agentfactory.io

### 6.5 Right to Data Portability

You can export your data in machine-readable format.

**How to Exercise:** Use data export API or contact us

### 6.6 Right to Object

You can object to processing of your data.

**How to Exercise:** Contact privacy@agentfactory.io

---

## 7. Data Retention

### 7.1 Retention Periods

- **Account Data:** Until account deletion
- **Usage Logs:** 90 days
- **Audit Logs:** 1 year
- **Billing Records:** 7 years (legal requirement)

### 7.2 Deletion

- You can delete your account at any time
- Data is deleted within 30 days
- Some data may be retained for legal compliance

---

## 8. Cookies and Tracking

### 8.1 Cookies We Use

- **Essential:** Required for Service operation
- **Analytics:** Help us understand usage
- **Preferences:** Remember your settings

### 8.2 Managing Cookies

- You can disable cookies in your browser
- Some features may not work without cookies
- See our Cookie Policy for details

---

## 9. International Data Transfers

### 9.1 Data Locations

- Data is stored in [Regions]
- We use Standard Contractual Clauses (SCCs) for transfers
- We ensure adequate protection

### 9.2 Transfer Mechanisms

- EU → US: Standard Contractual Clauses
- Other transfers: Adequacy decisions or SCCs

---

## 10. Children's Privacy

Our Service is not intended for children under 18. We do not knowingly collect information from children.

---

## 11. California Privacy Rights (CCPA)

California residents have additional rights:

- Right to know what personal information is collected
- Right to delete personal information
- Right to opt-out of sale of personal information
- Right to non-discrimination

**How to Exercise:** Contact privacy@agentfactory.io

---

## 12. Changes to Privacy Policy

- We may update this Privacy Policy
- We will notify you of material changes
- Continued use constitutes acceptance

---

## 13. Contact

**Agent Factory**  
**Founder, CEO & Operator:** Scott Hardie  
**Email:** scottrmhardie@gmail.com  
**Privacy Questions:** privacy@agentfactory.io  
**Data Protection Officer:** dpo@agentfactory.io  
**General Inquiries:** hello@agentfactory.io

**Company Information:**
- **Founder, CEO & Operator:** Scott Hardie
- **Location:** Greater Toronto Area, Canada
- **GitHub:** github.com/shardie-github
- **LinkedIn:** linkedin.com/in/scottrmhardie

---

## 14. Your Consent

By using our Service, you consent to this Privacy Policy.

---

**Note:** This is a template. Review with legal counsel, customize for your jurisdiction, and ensure compliance with applicable laws (GDPR, CCPA, etc.).
